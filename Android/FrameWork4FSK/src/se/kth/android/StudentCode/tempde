...
public class StudentCode extends StudentCodeBase {
    ...
    // Initialize cosines with specified frequency used for data transmission
    ...
    { 
           ...
           int length_rxb =no_samp_period*2000*24;
    ...
    {    
    	trigger=0;
    	// When tx button is pushed -> browse for file
    	if(side==0){
    		browseForFile();	
    	}
    }
    ...
    {  
    	...
    	if(trigger==2){
        	  ...
        	  int block_length=no_samp_period*(gb_length+ts_length+40); // Block used to do autocorrelation
        	  ...
        	  //send received data to goertzel2 algorithm, copy only data part
...
private double [] square(double [] in_values) {
 ...
 // Goertzel algorithm used for detection in 2FSK
...
// Correlation function used for general synchronization
 ...
 // Modulation function into 2FSK used to modulate the data
...
// Function that saves data to a text file
...
// Goertzel algorithm used for detection in 4FSK
...
// Modulation function into 4FSK used to modulate the data sequence
