...
public class StudentCode extends StudentCodeBase {
  ...
  // This is called before any other functions are initialized so that parameters for these can be set
	...
	{ 
		...
		projectName = "GENERATE SOUND";
	
		// Add sensors your project will use
		useSensors =   SOUND_OUT|SOUND_IN;// CAMERA;//CAMERA_RGB;//WIFI_SCAN | SOUND_OUT; //GYROSCOPE;//SOUND_IN|SOUND_OUT;//WIFI_SCAN | ACCELEROMETER | MAGNETIC_FIELD | PROXIMITY | LIGHT;//TIME_SYNC|SOUND_IN;//TIME_SYNC | ACCELEROMETER | MAGNETIC_FIELD | PROXIMITY | LIGHT | SOUND_IN;
		...
		introText = "This is the TX Client \n test_signal.dat will be played if you press Start button";
	...
	public void start()
	{	
		//load_sound_file("mod_signal.wav");
		//play_sound_file(1);
		playsoundexample();
  ...
  private double [] square(double [] in_values) {
	...
	public void playsoundexample(){
		if (init_done && (!file_loaded)) {
			the_sound_file_contents=read_data_from_file("test_signal.dat"); // Read file from plain file of samples in form of shorts
		...
		if (file_loaded) {
			buffer = new short[the_sound_file_contents.length/2];
