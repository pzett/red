    ...
    { 
    	...
    	case GETDATA:
    		bit_buffer = data_buffer_bits();    		
     		set_output_text("bit_buffer= "+bit_buffer);
    	...
    	if(trigger==2){
        	  ...
        	  for (int k=0;k<rx_buffer.length;k++){
        		  received_buffer[k] = (int) rx_buffer[k];
        	  }
        	  
        	  retrieveData(received_buffer);
        	  trigger=-1;
        	  //stop();
...
public int[] data_buffer_bits(){
	   if (init_done && (!file_loaded) && (!(d_filename==null))) {
			...
			//file_loaded=true;
			//set_output_text("data_buffer ="+the_file_contents_bb);
			file_loaded=true;
			};
		...
		if (file_loaded){
		//	data_buffer = new byte[the_file_contents_bb.remaining()];
		//	the_file_contents_bb.get(data_buffer, 0, data_buffer.length);
		//	the_file_contents_bb.clear();
			numberBits = 8*the_file_contents.length;
			
			data_buffer_bits = new int[numberBits];
			data_buffer = new byte [the_file_contents.length];
			//data_buffer = new byte[the_file_contents_bb.capacity()];	
			//the_file_contents_bb.get(data_buffer);
			//set_output_text("data_buffer ="+data_buffer[1]);
		for (int k=0;k<the_file_contents.length;k++) {				
			data_buffer[k]=the_file_contents_bb.get(k); // Create a buffer of bytes
			//set_output_text("data_buffer ="+data_buffer[1]);
			//System.out.println("data_buffer="+data_buffer[0]);
		    //data_buffer_bits[8*k]= (int) data_buffer[k];
			
			//data_buffer_bits[k]= data_buffer[k];
		    for (int k1=0; k1<8; k1++){
			data_buffer_bits[8*k+k1]=(data_buffer[k] >> (7-k1) & 1);
			//set_output_text(""+data_buffer_bits[8*k+k1]);
			//System.out.println("data_buffer_bits ="+data_buffer_bits[0]);
			};
		}
		}		
		side=5;
...
public void retrieveData(int[] received){
	   ...
	   //int m;
	   byte[] data_buffer_received = new byte[data_buffer.length];
	...
	//	   m=7;
		 ...
		 //  data_buffer_received[k] = (byte) data_temp[k];
		  ...
		  // int receivedBitstemp_test = receivedBitstemp[0] receivedBitstemp[1] receivedBitstemp[2] ;
		   data_buffer_received[k]= Byte.parseByte(data_concatenated,2);
	   }
	   ...
	   //Create file
		...
		try {
			...
			outFile.write(data_buffer_received);
			outFile.close();
			//state=2;
		} catch (IOException e) {
			e.printStackTrace();
