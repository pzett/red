function [ decision ] = goertzel2amp( w1,w2,w3,w4,Fs,n,r,ref)
...
decision=zeros(1,4*length(mag1));
for(k=1:length(mag1))
...
amp_comp=amp(k)/ref
        ...
        if(amp_comp<1-1/8 && amp_comp>3/4-1/8)
            decision(2*aux+1)=1;
            decision(2*aux+2)=0;
        end
        
        if(amp_comp<3/4-1/8 && amp_comp>2/4-1/8)
            decision(2*aux+1)=1;
            decision(2*aux+2)=1;
        end
        if(amp_comp<1/4+1/8)
