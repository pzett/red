...
Vnx2=Vnx2(1:n_sym*(length(data)/(2*levels)+ts_length+20));
Vny2=Vny2(1:n_sym*(length(data)/(2*levels)+ts_length+20));
subplot(2,2,3)
pwelch(Vnx2)
...
% 
% %%
% errors = sum(data(length(ts)+1:length(decoded))' ~= decoded(length(ts)+1:end))
% stem(data(length(ts)+1:length(decoded))' ~= decoded(length(ts)+1:end))
% %errors = sum(data(ts_length*2*levels:a)' ~= decoded(ts_length*n_sym:end))
%  BER = errors / a* 100
%  R=2*levels / Ts
 test =[ts; data];
            if(length(mdem)>=length(test))
                decoded=mdem;
...
scatterplot(mconstdem(ts_length:end)); grid on ; xlabel('I'); ylabel('Q'),title('Received Constellation after Rotation and Offset Correction');
