...
%     pwelch(real(r),[],[],[],fs); title('PSD of received signal after (x) with complex exponential')
    ...
    if(file)
        ...
        if(intlv);  st2 = 4831; file_data = randdeintrlv(file_data(1:length(data_sent),st2)); end;
        % file_data=deinterleaving(deinterleaving(file_data,9,11),9,11);
        figure(8)
        errors = sum(test(length(ts)+1:end) ~= file_data(1:length(data_sent))');
       ...
       % write_file(file_data);
        ...
        if(code)
            ...
            if(length(demodulated)>=length(test))
                ...
                if(intlv)
                    st2 = 4831;
                    demodulated = randdeintrlv(demodulated,st2);
                end
