            ...
            for(k_alfa=1:length(alfa))
                for(k_t=1:length(t_block))
                    for(k_fchard=1:length(fchard))
                        ...
                        %h1 =  eyediagram(Hx(400:420)+1i*Hy(400:420),4,1/f1,0);
                        ...
                        if(plotting) plot((Hx(200:800)+1i*Hy(200:800))*exp(-1i*phihat)/ref); grid on; xlabel('I'),ylabel('Q'),title('Symbol transistions');
                        end
