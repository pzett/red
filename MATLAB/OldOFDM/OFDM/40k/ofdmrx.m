...
ref2 = 1;
batch_length = 64;
if(mod(pilot_int/(2*levels),batch_length) ~= 0 ); disp('Pilot interval and batch length should match !'); pause; end
...
for(k=1:floor(length(mconst)/batch_length))
    ...
    if(plotting); plot(real(mconst_phi),imag(mconst_phi),'.'); drawnow; end
