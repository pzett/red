...
% decoded = decoded / max(abs(decoded)) * ((levels-1)*2 +1)
%    max_sync = 1 / (sum((real(decoded(1:ts_length))-real(transpose(mconst_ts))).^2 + ...
%         (imag(decoded(1:ts_length))-imag(transpose(mconst_ts))).^2 )/ length(mconst_ts));
