...
[k_start nr_of_bits ref] = pilot_demodulator(r,mod_ts,gb_length,Tb,fs,f1_ts,f2_ts,d_length);

nr_of_samples = 1/4*nr_of_bits*Tb*fs; %number of samples expected to be received 

fprintf('Expected number of bits to be received: %d \n',nr_of_bits);

r=r(k_start:k_start+nr_of_samples-1);%crop r to the data containing sequence
length(r)
...
decision=goertzel2amp(f(1),f(2),f(3),f(4),fs,Tb*fs,r,ref);%decide using Goertzel Algorithm


% FDM
% decision1=goertzel2(f1(1),f1(2),f1(3),f1(4),fs,Tb*fs,r);%decide using Goertzel Algorithm
% decision2=goertzel2(f2(1),f2(2),f2(3),f2(4),fs,Tb*fs,r);%decide using Goertzel Algorithm
% decision3=goertzel2(f3(1),f3(2),f3(3),f3(4),fs,Tb*fs,r);%decide using Goertzel Algorithm
% decision4=goertzel2(f4(1),f4(2),f4(3),f4(4),fs,Tb*fs,r);%decide using Goertzel Algorithm
% decision5=goertzel2(f5(1),f5(2),f5(3),f5(4),fs,Tb*fs,r);%decide using Goertzel Algorithm
% decision6=goertzel2(f6(1),f6(2),f6(3),f6(4),fs,Tb*fs,r);%decide using Goertzel Algorithm
% decision7=goertzel2(f7(1),f7(2),f7(3),f7(4),fs,Tb*fs,r);%decide using Goertzel Algorithm
%decision8=goertzel2(f8(1),f8(2),f8(3),f8(4),fs,Tb*fs,r);%decide using Goertzel Algorithm
...
%---------Plots--------------------------
%%
%errors = sum(decision(1:length(data))~=data);
...
%errors8 = sum(decision8(1:length(data))~=data(8,:));

%errors = errors1 + errors2 + errors3 + errors4 + errors5 + errors6 + errors7;
errors=sum(decision(1:nr_of_bits)~=data);
BER = errors/(Nb)*100;
