...
Tb=25/fs;  %Bit/symbol period + guard period between symbols
t = Tb;    %Bit/symbol period (t=Tb to set guard period to 0)
Mf=4;       %Number of frequencies to transmit over. 
Ma=4;
f=zeros(1,Mf); %vector containing the frequencies to be used.
alfa=zeros(1,Mf); %variable to construct the working frequencies
...
bits_per_symbol = log2(Mf+Ma); %bits transmitted per symbol
...
save('MFSK.mat','f','Tb','t','bits_per_symbol','Mf') %save variables for 
                                                    %testing purposes    
...
f2_ts = 1/multiple*fs;
f3_ts = 8/multiple*fs; % Frequencies of FSK modulated training sequence
f4_ts = 6/multiple*fs;
...
% ----------Modulation------------
[mod_pilot mod_ts ref] = pilot_modulator(tsequence,Nb,gb_length,Tb,fs,f1_ts,f2_ts,f3_ts,f4_ts,d_length);
save('mod_ts.mat','mod_ts')

%mod_signal = FSK_modulation(data,fs,f1,f2,Tb,gb_length,tsequence);  
%mod_signal = MFSK_modulation(data);
mod_signal = MFSKwindow_modulation(data, fs,f,Tb,gb_length,t,tsequence,Mf+Ma,ref);
